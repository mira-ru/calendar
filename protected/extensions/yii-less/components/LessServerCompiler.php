...
class LessServerCompiler extends LessCompiler
{
	...
	public function run()
	{
		...
		foreach ($this->files as $lessFile => $cssFile)
		{
					...
					|| filemtime($lessPath) > filemtime($cssPath))
			{
				...
				if (!file_exists(dirname($this->basePath . DIRECTORY_SEPARATOR . $cssFile)) )
					mkdir(dirname($this->basePath . DIRECTORY_SEPARATOR . $cssFile), 0755, true);
	...
	protected function compileFile($lessPath, $cssPath)
	{
		...
		$command = $this->nodePath . ' ' . $this->compilerPath . ' -x ' . implode(' ', $options) . ' ' . $lessPath . ' > ' . $cssPath;
