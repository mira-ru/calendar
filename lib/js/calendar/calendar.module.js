(function(require){
	...
	Calendar.initialize = function(){
		...
		$('body').on('click','.-button:not(.-button-green)', function(){
			var data = $(this).parents('form').serializeArray();
			yaCounter22425796.reachGoal('order')
			$.ajax({
				url: '/site/signUpEvent',
				type: 'post',
				dataType: 'json',
				data: data,
				success: function(response) {
					if (response.success) {
						$('#modalForm .modal-body').html('<p>Вы успешно записались</p>');
					} else {
	...
	function _getEvents(data) {
		...
		request.done(function(msg) {
			...
			if(Calendar.state.type != 'center' && Calendar.state.type != 'service'){
				...
				$(period[1]).attr('href','/c/' + msg.week.prev + params).attr('data-time', msg.week.prev);
				$(period[2]).attr('href','/c/' + msg.week.next + params).attr('data-time', msg.week.next);
			}
	...
	function _resetFilter() {
		$('#filter').empty();
		Calendar.setOptions({type:'center', item:Calendar.state.center_id, search: false});
